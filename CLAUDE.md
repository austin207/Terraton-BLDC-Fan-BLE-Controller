...
- **Setting any sleep timer clears Smart:** `case TIMER` does `smart_mode = 0` for codes 2/4/8. A user setting a timer while in Smart silently leaves Smart — a firmware feature conflict, not an app bug.
- **The status poll emits a stale extra frame.** `case HEARTBEAT` calls `send_status()` then `break`s into `Process_Response()`'s unconditional trailing `calculate_crc(); send_response();`, re-sending whatever `response_frame[]` still held — stale `cmd`, stale `len`, **freshly valid CRC**. So the fan is a live source of well-formed frames that answer nothing, once per 3 s poll. This is precisely what `MachineStateSync`'s two-reply agreement rule exists to reject; do not "simplify" that rule on the assumption that a valid checksum implies a real answer. It is also the likely cause of the documented RPM checksum quirk (`0x24` is the only 2-byte payload, and `calculate_crc()` appears not to read `response_frame[4]`).
