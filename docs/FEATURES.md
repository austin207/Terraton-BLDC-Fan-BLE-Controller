...
| **Fan remotes** | Three ceiling-fan remotes selected by model: **CF-01** (default — no light), **CF-02** (LED toggle replaces Nature), **CF-03** (Smart · Reverse · Boost, adds CoolLight, no Timer-OFF button). Chosen at pairing (Automatic Connect) or from the QR payload; switchable later via long-press → *Change Remote*. See `RemoteProfile` / `appliances.yaml`. |
| **Nature mode** | Locks the speed dial and disables other modes; restores pre-nature speed on switch to Smart/Reverse |
| **CoolLight** | ON/OFF + brightness slider on the integrated downlight *(bytes pending from Terraton)*. Warm/Neutral/Cool colour-temperature selection is built but dormant (`showColorTemp`). |
