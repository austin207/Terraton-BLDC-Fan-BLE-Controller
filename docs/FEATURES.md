...
| **Fan remotes** | Three ceiling-fan remotes selected by model: **CF-01** (default — no mood light), **CF-02** (LED toggle replaces Nature), **CF-03** (Smart · Reverse · Boost, adds mood light). Chosen at pairing (Automatic Connect) or from the QR payload; switchable later via long-press → *Change Remote*. See `RemoteProfile` / `appliances.yaml`. |
