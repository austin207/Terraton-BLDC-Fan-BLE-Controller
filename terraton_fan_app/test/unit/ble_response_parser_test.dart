...
void main() {
  ...
  group('BleResponseParser.parse', () {
    ...
    test('returns null for wrong packet id (e.g. 0x06 request frame)', () {
      // (0x55+0xAA+0x06+0x02+0x01+0x01) & 0xFF = 0x09
  ...
  group('BleResponseParser.parseAll', () {
    ...
    test('non-0x07 packet id is rejected', () {
      // Request frame (0x06) should not be parsed as a response.
      // (0x55+0xAA+0x06+0x04+0x01+0x03) & 0xFF = 0x0D
      expect(
        BleResponseParser.parseAll([0x55, 0xAA, 0x06, 0x04, 0x01, 0x03, 0x0D]),
        isEmpty,
      );
    });
  });
